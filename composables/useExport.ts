...
export function useExport() {
  ...
  const complexity = computed(() => {
    let c = store.enabled.core._self ? 5 : 0 // Core personality (5 traits)
  ...
  function exportFullConfig() {
    const agentConfig: Partial<AgentConfig> & { version: string; metadata: AgentConfig['metadata']; enabled: typeof store.enabled } = {
      version: '2.0',
      metadata: {
    ...
    if (store.enabled.core._self) {
      agentConfig.core = { ...store.core }
      agentConfig.facets = { ...store.facets }
    }
...
Generated by Agent Crafter v2.0`)
  ...
  function generateAgentSummary(): string {
    ...
    if (store.enabled.core._self) {
      ...
      personalityOverview = `This agent combines ${extLevel} extraversion${store.enabled.communication._self ? ` with ${store.communication.primaryStyle} communication style` : ''}.`
    } else if (store.enabled.communication._self) {
      personalityOverview = `This agent uses ${store.communication.primaryStyle} communication style.`
    }
